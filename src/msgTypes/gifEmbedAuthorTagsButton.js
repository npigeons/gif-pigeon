...
export function isMsgGeneratedByThisView(msg) {
	...
	return msg.embeds.length === 1 && msg.embeds[0].data.image && msg.components.length === 1 && msg.components[0].components.length === 1;
}
...
export function getMessageOptions(gif, tags) {
	...
	const components = [new ActionRowBuilder().addComponents(suggestTagsButton)];
